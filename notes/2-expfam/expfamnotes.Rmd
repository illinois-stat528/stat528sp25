...
$$
  D(y;\hat\mu) \; > \: \chi^2_{n-p}(1-\alpha), 
$$
where $\chi^2_{n-p}(1-\alpha)$ is the $(1-\alpha)$ quantile of a chi-squared random variable with $n-p$ degrees of freedom. Note that the $\chi^2$ approximation can be poor in practice.
...
BIC never selects larger models than AIC.
...
To do that, we use the method of [Lagrange multipliers](https://en.wikipedia.org/wiki/Lagrange_multiplier). Multiply each constraint function by a new parameter (Lagrange multiplier) and add to the objective function. This gives the Lagrangian function
\begin{align*}
...
$$
  \nabla^2 c_{\text{sub}} c(M\beta) = M^T\left(\nabla c(M\beta)\right)M
$$
