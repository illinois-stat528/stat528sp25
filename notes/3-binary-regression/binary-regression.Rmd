...
\begin{align*}
  \Prob(Y = 1\mid X = x) &= \Prob(x^T\beta_1 + \varepsilon_1 > x^T\beta_0 + \varepsilon_0\mid X = x) \\
    ...
    &= \Phi(x^T\beta^*)
\end{align*}
where $\beta^* = (\beta_1 - \beta_0)/\sqrt{2}$. Again, this is the probit model. 

The probit model can extend to an inverse $t$ link, for which corresponding latent variable models can better accommodate outliers.
...
Let $x_{ij}$ denote the value of predictor $j$ for subject $i$. For the probit model $\Phi^{-1}(p(x_i)) = \sum_{j=1}\beta_jx_{ij}$, the log likelihood function is
$$
  l(\beta) = \log\left\{\prod_{i=1}^n\left[\Phi\left(\sum_j \beta_jx_{ij}\right)\right]^{y_i}
  \left[1 - \Phi\left(\sum_j \beta_jx_{ij}\right)\right]^{1 - y_i}\right\}.
$$
Differentiation with respect to $\beta_j$ leads to,
$$
  \sum_{i}\left\{\left[\frac{\left[y_i - \Phi\left(\sum_j\beta_j x_{ij}\right)\right]x_{ij}}
...
$$
with $\phi(\cdot)$ being the standard normal pdf.
...
$$
  w_i =\frac{\left[\phi\left(\sum_j\hat\beta_j x_{ij}\right)\right]^2}
...
Note that the AIC value corresponding the probit model is slightly smaller than the AIC value corresponding to the logistic model. This provides some support for the probit model. However, selecting this model would mean losing the many nice properties offered by exponential models. Furthermore, the [relative likelihood](https://en.wikipedia.org/wiki/Relative_likelihood#Relative_likelihood_of_models) of the logistic model is close to 1 which suggests that the probit model's advantage in AIC is not very reliable.
...
\vspace{5pt}
We see that that the \cite{esarey2012assessing} diagnostic suggests that the estimated conditional success probabilities obtained from the probit model reasonably approximate the frequency of observed successes across the covariates. This is not surprisng given that the probit and logistic models have very similar fits to this data.
\vspace{5pt}
...
This example will highlight the logistic regression as a model for treatment assignment in [causal inference](https://en.wikipedia.org/wiki/Causal_inference). In this example we will only scratch the surface of casual inference and will cover important causal estimators in very little detail. External references that go into more details about causal inference will be provided. Feel free to read them if you are interested. The goal of our data analysis will be to determine if course lectures provide any improvement to student exam scores when an online course implementation already exists.
...
The estimators $\widehat{\text{ATE}}$ and $\widehat{\text{ATE}}_{\text{alt}}$ are consistent if the propensity score model is correctly specified, so should be approximately unbiased in finite samples. We will also consider a [double robust (DR) estimator](https://pmc.ncbi.nlm.nih.gov/articles/PMC3070495/) of the ATE. This estimator is
$$
...
$$
where $m_a(X,\alpha_a) = E(Y|A=a,X)$ is the outcome regression of the response on $X$ in each level of $A \in \{0,1\}$, depending on parameters $\alpha_a$, and $\hat{\alpha}_a$ is the estimator for $\alpha_a$ based on the data from subjects with $A = a$. The double robust estimator is unbiased for the average treatment effect provided only one of the outcome or propensity score models are correctly specified.
