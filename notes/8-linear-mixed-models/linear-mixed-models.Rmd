...
This is a demonstration from a [paper](https://onlinelibrary.wiley.com/doi/full/10.1111/tpj.17219) on improving the efficiency of crop photosynthesis via genetic manipulation to speed up the relaxation of photoprotective mechanisms during sun-to-shade transitions.

In this demonstration the researchers are interested in determining which genotypes are associated with a different expression of apparent quantum efficiency (AqE) from the RC reference genotype. AqE is a part of the photosynthetic process in plants where improvements in AqE allow for improvements in yield. The researchers have collected data across multiple plots for multiple days over two years. The plot effects are not of interest.

It is known that AqE will have a quadratic effect in date, and that AqE may depend on weather conditions. Some weather variables are also collected (Fsd\_7day, Ta\_7day, Precip\_7day, and Precip\_cum).
...
Our procedure for determining influential genotypes for each response will follow these steps:
...
We now compare the mixed-effects model containing a random effect for plots with a larger mixed-effects model containing fixed effects for weather variables. Again, the parametric bootstrap is fairly time-consuming. A quicker decision can be made using model selection criteria. Both AIC and BIC select the largest model with random effects for plots and fixed effects for weather variables.
...
We see that the modeling assumptions of constant variance and normality of residuals are satisfied in all candidate models.
...
Note that \texttt{lmer} does not provide p-values for fixed-effects. Per Ben Bolker, one of the developers of \texttt{lmer}, not printing p-values has been a "feature" of \texttt{lmer} forever (see a formal technical report on LMM and GLMM on this issue [here](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#why-doesnt-lme4-display-denominator-degrees-of-freedomp-values-what-other-options-do-i-have) and Doug Bate's less than formal take [here](https://stat.ethz.ch/pipermail/r-help/2006-May/094765.html)).
...
We see that the fields do seem to vary as the result is significant.
