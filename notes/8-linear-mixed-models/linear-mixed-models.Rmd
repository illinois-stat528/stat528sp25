...
So far in this class, we have discussed models that include fixed effects only. In such models, the regression parameters are interpreted as population-level or sub-population-level quantities, and our inference is directed toward these fixed parameters. We now introduce a simple example of a mixed-effects model, which includes both fixed and random effects. Our illustrative example is a two-way ANOVA model with the following structure:
$$
  y_{ijk} = \mu + \tau_i + v_j + \varepsilon_{ijk}
$$
where $\mu$ is a population mean fixed effect, $\tau_i$ is a sub-population fixed-effect, $v_j$ is a random effect, and $\varepsilon_{ijk}$ is an error term. We will assume that $v_j \overset{iid}{\sim} N(0,\sigma^2_v)$ and $\varepsilon_{ijk} \overset{iid}{\sim} N(0,\sigma^2)$ where $v_j$ and $\varepsilon_{ijk}$ are independent. In this model, we are typically interested in:

  - Estimating and testing the fixed effects $\tau_i$, for example, testing $H_0: \tau_i = 0$.

  - Estimating the variance component $\sigma^2_v$ and testing $H_0: \sigma^2_v = 0$.

Notice that for fixed effects (like the $\tau_i$), we estimate each parameter and test them individually. For the random effects, we are usually interested in estimating and testing the variance component $\sigma^2_v$ as a whole, rather than the individual $v_j$ terms.


# Basic Linear Random Effect Model Example

Let’s now consider the simplest possible random effects model: a [one-way ANOVA](https://en.wikipedia.org/wiki/One-way_analysis_of_variance) with random group effects. This is a special case of the model above, where there is only one factor with $a$ levels:
$$
  y_{ij} = \mu + \alpha_i + \varepsilon_{ij}; \quad i = 1,\ldots,a; \quad j = 1,\ldots,n,
$$
where the $\alpha$'s and the $\varepsilon$'s have zero mean and respective variances $\sigma^2_\alpha$ and $\sigma^2$. These variances are known as the \textbf{variance components}. Note that this formulation induces an intraclass correlation between observations at the same level that is formally expressed as
$$
  \rho = \frac{\sigma^2_\alpha}{\sigma^2_\alpha + \sigma^2}.
$$
This intraclass correlation quantifies the proportion of total variance that is attributable to differences between groups, and it is an important feature of random effects models.

To analyze this model using ANOVA techniques, and for simplicity, assume that each group has an equal number of observations, $n$ per level. The total variation in the data can be decomposed as sum-of-squares total (SST) = sum-of-squres within groups (SSE) + sum-of-squares between groups (SSA),
$$
  \sum_{i=1}^a\sum_{j=1}^n(y_{ij} - \bar{y}_{++})^2  
    = \sum_{i=1}^a\sum_{j=1}^n(y_{ij} - \bar{y}_{i+})^2
      + \sum_{i=1}^a\sum_{j=1}^n(\bar{y}_{i+} - \bar{y}_{++})^2,
$$
where $\bar{y}_{i+}$ is the sample mean within group $i$ and $\bar{y}_{++}$ is the overall mean across all observations,

Dividing through by the respective degrees of freedom, we obtain the mean squares, MSE and MSA. Now we find that
$$
  E(SSE) = a(n-1)\sigma^2, \qquad  
  E(SSA) = (a-1)(n\sigma^2_{\alpha} + \sigma^2)
$$
which use the estimators: 
$$
  \hat\sigma^2 = \frac{SSE}{a(n-1)} = MSE, \qquad 
  \hat\sigma^2_\alpha = \frac{SSA/(a-1) - \hat\sigma^2}{n} = \frac{MSA - MSE}{n}.
$$
...
We now turn to the general framework for linear mixed models under normality assumptions. The model is given by:
$$
...
MLE also suffers bias when estimating random effect parameters. Recall a simple example where $x_1,\ldots,x_n\overset{iid}{\sim} N(\mu,\sigma^2)$, then the MLE of $\sigma^2$ is $\hat\sigma^2 = n^{-1}\sum_i(x_i - \bar x)^2$. This estimate is biased, an unbiased estimator is leading term should be $(n-1)^{-1}$ instead of $n^{-1}$. Conceptually, these problems are driven by estimation of $\mu$. If $\mu$ were known then $n^{-1}\sum_i(x_i - \mu)^2$ is an unbiased estimator for $\sigma^2$. Similar problems occur with the estimation of variance components. Given that the number of observations for some factor may not be large, the bias of the MLE for the variance component associated with that factor may be quite large.

\textbf{Restricted maximum likelihood estimation (REML)} estimators are an attempt to get around problems with standard unrestricted maximum likelihood estimation. The idea of REML is to find all linear combinations, denoted $k$, such that $k^TX = 0$. Form a matrix $K$ with columns $k$, so that 
$$
  K^TY \sim N(0, K^TVK).
$$
...
## Example: Brightness of Paper Pulp Depending on Shift Operator
...
## Inferential Challenges for Nested Models
...
## Example: Brightness of Paper Pulp Depending on Shift Operator (continued)
...
where:
 ...
 - $D = \Var(b)/\sigma^2$,
 - $Z$ is the fixed random-effects model matrix,
 - $V = I + ZDZ^T$,
 - $Y - X\beta$ are the residuals corresponding to a regression on the fixed-effects.

A purely Bayesian approach would specify the parameters of the prior (or specify priors for these) and compute a posterior distribution for $\alpha$. We can also take an empirical Bayesian point of view and substitute the MLEs into $D$, $V$, and $\beta$ to obtain predicted random-effects. This is the approach implemented in \texttt{lme4} for obtaining $\hat{\alpha}$ in \eqref{eq:rep}:
...
Both irrigation and plant variety are fixed effects, but field should be modeled as a random effect (\textbf{Why so?}). We can also consider the interaction between field and variety, which is necessarily also a random effect because one of the two components is random. The fullest model that we might consider is:
$$
...
However, when you try to fit such a model, it will fail because it is not possible to distinguish the variety within field variation. We would need more than one observation per variety within each field for us to separate the two variabilities. We are forced to resort to a simpler model that omits the variety by field interaction random effect:
$$
...
Note that \texttt{lmer} does not provide p-values for fixed-effects. Per Ben Bolker, one of the developers of \texttt{lmer}, not printing p-values has been a "feature" of \texttt{lmer} forever. For further discussion, see this [formal technical FAQ]((https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#why-doesnt-lme4-display-denominator-degrees-of-freedomp-values-what-other-options-do-i-have)) and Doug Bates’s [informal and colorful explanation](https://stat.ethz.ch/pipermail/r-help/2006-May/094765.html).

While this omission may seem surprising, it is a deliberate design choice rooted in the nature of mixed models. In classical linear models, p-values are derived from F-tests with clearly defined numerator and denominator degrees of freedom, based on a known distribution of a residual variance estimate. However, in mixed models the residual variance is estimated via REML, and its distribution is not analytically tractable—particularly when random effects are present and variance components are themselves estimated. 

Although \texttt{lmer} uses a shared penalized residual variance across all fixed-effect tests, some software (like SAS) assigns different denominator degrees of freedom to each test using approximations (e.g., Satterthwaite or Kenward-Roger). Doug Bates, the author of \texttt{lmer}, considers this approach conceptually inconsistent: if the denominator variance is common across tests and its distribution is not known, assigning different degrees of freedom per test may give a false sense of precision. As a result, \texttt{lmer} omits p-values altogether. One can make model comparisons with formal inference available through companion tools like \texttt{pbkrtest}.

Returning to our analysis, we can see that the largest variance component is due to the field effect, with $\hat\sigma_f = 4.03$ compared to $\hat\sigma_\varepsilon = 1.45$. The relatively large standard errors, in comparison to the fixed effect estimates, suggest that there may be no significant fixed effects. We can check this sequentially using a backwards model selection procedure with [Kenward-Roger approximate F-tests](https://pubmed.ncbi.nlm.nih.gov/9333350/) that employ adjusted degrees of freedom and are implemented in the previously mentioned \texttt{pbkrtest} package.
...
We can see that there is no problem with the nonconstant variance, but that the residuals indicate a bimodal distribution caused by the pairs of observations in each field. This type of divergence from normality is unlikely to cause any major problems with the estimation and inference. We can test the random effects using the \texttt{exactRLRT} function which performs a likelihood ratio test based on simulated values from the finite sample distribution for testing whether the variance of a random effect is 0 in a linear mixed model with known correlation structure of the tested random effect and iid errors:
...
We see that yields does seem to vary across fields as the result is significant.


# Example: Modeling Photoprotection Dynamics in Soybean

This example is a mixed-effects modeling demonstration adapted from a [recent study](https://onlinelibrary.wiley.com/doi/full/10.1111/tpj.17219) on improving the efficiency of crop photosynthesis via genetic manipulation to speed up the relaxation of photoprotective mechanisms during sun-to-shade transitions.

n this demonstration, the researchers aim to identify genotypes that differ in their expression of apparent quantum efficiency (AqE) relative to a reference genotype (RC). AqE is a component of the photosynthetic process, and improvements in AqE can potentially lead to increased yield. The dataset includes repeated measurements across multiple plots, days, and two field seasons. Plot effects are treated as nuisance variation.

It is known that AqE exhibits a quadratic trend over the growing season (date), and may also depend on environmental conditions. Several weather-related covariates are available: (Fsd\_7day, Ta\_7day, Precip\_7day, and Precip\_cum).
...
```{r}
pairs(dat %>% select(mAqE, Fsd_7day, Ta_7day, Precip_7day, Precip_cum))
```

We will now fit and compare models under three circumstances: 

1. model fit to all data with plot random effects
2. model fit to 2021 data with plot random effects
3. model fit to 2022 data with plot random effects

We will consider two functional forms for weather variables and date variables: 

1. quadratic model in date only
2. quadratic model in date with main effect terms for additional weather variables (denoted full model)

This gives a total of six candidate models fit in a common order: 
...
The goal of this modeling framework is to identify genotypes whose expression of AqE differs meaningfully from the reference line (RC). Our procedure was designed to balance principled statistical modeling with a highly selective threshold for declaring genotype influence, reflecting the level of caution and robustness the researchers deemed necessary for biological interpretation.

Our selection pipeline proceeds as follows:

1. Investigate model diagnostics for the six candidate models. Refit with transformations if needed.
2. Determine if each genotype provides a better fit over reference level through a leave-one-variable-out AIC procedure. We refit the model with the genotype under investigation removed and record AIC values for this model and the original model with the genotype included. The models are fit so that the genotype effect is interpreted as its difference from the RC reference level (the RC level is encoded in the model intercept). 
3. Obtain coefficient estimates for the genotype under study.
4. Report genotypes that have consistent sign estimates and are determined to differentiate from the reference level across all six candidate models. These are the genotypes that we call influential.
...
Just as before we can use a parametric bootstrap to compare nested models. In this case we compare the linear regression model to a mixed-effects model containing a random effect for plots. In this example we can see that the parametric bootstrap is prohibitively time-consuming.
...
We now compare the mixed-effects model containing a random effect for plots with a larger mixed-effects model containing fixed effects for weather variables. Given the computational cost of the parametric bootstrap, we could proceed with model selection criteria such as AIC and BIC, which in this case strongly support the more complex model with random effects for plots and fixed effects for weather variables.
...
Below we see that the modeling assumptions of constant variance and normality of residuals are satisfied in all candidate models.
...
We now report influential genotypes according to our selection procedure. 
...
## Algebraic and Computational Accounting of Mixed-Model Input Formulas
...
The $q \times q$ covariance factor $\Lambda_\theta$ is a block diagonal matrix whose $i$th diagonal block $\Lambda_i$ is of size $q_i$, $i=1,\ldots,k$. We refer to $\Lambda_i$ as the \emph{term-wise relative covariance factor}. 
