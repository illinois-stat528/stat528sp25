...
$$
  \E(\tilde y_i|\beta,b) \approx x_i^T\beta + z_i^Tb.
$$ 
This transforms the GLMM fitting problem at iteration $k$ into a weighted linear mixed model problem. To apply standard LMM machinery, we also approximate the conditional variance $\mathrm{Var}(\tilde y_i^k \mid b)$ using exponential family theory and a first-order Taylor expansion around the current estimates. This yields the working weights used in each iteration of the IRLS algorithm (see derivation below).
...
\begin{itemize}
\item A data model $p(y_i|\theta_i,\psi_1)$ where $\psi_1$ are likelihood-specific hyperparameters (dispersion for example).
\item A latent Gaussian field $\theta|\psi_2 \sim N(\mu(\psi_2), Q(\psi_2)^{-1})$ where $Q$ is a precision matrix. Precision matrices are inverse covariance matrices and they encode conditional independence which is relevant in this modeling formulation.
\item Low-dimensional hyperparameters $\psi = (\psi_1,\psi_2)$
\end{itemize}
...
The code below installs the \texttt{INLA} package.
...
See Section 12.2 in \cite{faraway2016extending} for an introduction. For ease of exposition, we use only the surface and vision as fixed effect predictors. The default, noninformative priors, are satisfactory:
...
When analyzing correlated or clustered data, GLMMs offer a powerful framework by explicitly modeling subject-specific variability through random effects. However, GLMMs come with several limitations. They require full distributional assumptions for the response and random effects, can be computationally intensive, and their fixed effects are interpreted conditionally on the random effects. This interpretation may not always align with scientific goals focused on population-level inference.

As an alternative, we now consider generalized estimating equations (GEE), which retain the structure of generalized linear models while relaxing the need to fully specify the joint distribution. GEE offers a flexible approach to estimating marginal (population-averaged) effects in the presence of correlation. It only requires specifying a link function and a variance function relating the mean to the variance, avoiding the strong distributional assumptions required by GLMMs. In contrast, the fixed effects in a GLMM describe changes in the response \emph{conditional} on the random effects. Recall that
$$
  g^{-1}\left(\mathbb{E}[Y_i \mid b_i]\right) = x_i^T\beta + z_i^T b_i.
$$
Thus, fixed effects in a GLMM quantify how predictors influence the response for a given individual or group, holding the unobserved random effect constant.


<!-- The random effects themselves can be viewed as shrinkage estimates of what one would obtain by treating each group as a fixed effect (as seen in the LMM notes, particularly the section on predicting random effects). These estimates borrow strength across clusters, pulling extreme group-level estimates toward the overall mean and improving inference in settings with sparse or unbalanced data. -->

We now describe the methodological details of GEEs. We will let $Y_i$, $i = 1,\ldots,N$ be a vector of random variables representing the responses on a given individual or cluster. Let
$$
  E(Y_i) = \mu_i  
$$  
which is then linked to the linear predictor using 
$$
\mu_i = g(x_i^T\beta)
$$ 
where $g$ is a change of parameters mapping. We will also specify a variance function $a$ that satisfies
$$
...
We also must specify how the responses within an individual or cluster are correlated with each other. We set a \emph{working correlation matrix} $R_i(\alpha)$ depending on a parameter $\alpha$ which requires estimation. This yields a \emph{working covariance matrix} for $Y_i$ denoted as  
$$
  V_i = \phi A_i^{1/2}R_i(\alpha)A_i^{1/2},
$$
where $A_i$ is a diagonal matrix formed from $a(\mu_i)$.

The "sandwich" form ensures that $V_i$ has the correct marginal variances (from the variance function $a(\mu_i)$) while imposing a structured pattern of correlation across responses within cluster $i$ (from $R_i(\alpha)$). So the full structure of $V_i$ reflects both the mean–variance relationship and the within-cluster correlation.

Provided estimates of $\phi$ and $\alpha$, we can estimate $\beta$ by setting the multivariate score function equal to zero and solving the generalized estimating equation
$$
  \sum_{i=1}D_i^TV_i^{-1}(Y_i - \mu_i) = 0,
$$
where $D_i = \frac{\partial \mu_i}{\partial \beta}$ \citep{liang1986longitudinal}. This estimating equation resembles the score equation from GLMs, where we set the derivative of the log-likelihood to zero. In GEE, this idea is generalized by constructing a score-like equation based on the mean and covariance structure, without requiring a full likelihood.

Since variances also depend on $\alpha$, we can substitute any $\sqrt{N}$ consistent estimate of $\alpha$ in this equation and still obtain an estimate as asymptotically efficient as if $\alpha$ were known. We denote this estimator by $\hat\alpha = \hat\alpha(\beta, \phi)$. Except for particular choices of $R$ and $\alpha$, the scale parameter $\phi$ will generally remain in the GEE. To complete the process, we replace $\phi$ by $\hat\phi(Y,\beta)$, a $\sqrt{N}$ consistent estimator when $\beta$ is known. Let $U_i(\beta, \alpha) = D_i^TV_i^{-1}(Y_i - \mu_i)$, and with these estimators we have
\begin{equation} \label{eq:GEE}
  \sum_{i=1}^N U_i(\beta, \hat\alpha(\beta, \hat\phi(\beta)) ) = 0.
\end{equation}
The above estimation \eqref{eq:GEE} can be performed iteratively where one alternates between estimating $\beta$ with $(\hat\alpha,\hat\phi)$  fixed, and estimating $(\alpha,\phi)$ with $\hat\beta$ fixed. 

The solution of the \eqref{eq:GEE}, denoted $\hat\beta$, has an asymptotic multivariate normal distribution of the form
$$
...
$$
Replacing $\beta$, $\phi$, and $\alpha$ by consistent estimates and the covariance matrix $\text{Cov}(Y_i)$ by $(Y_i - \mu_i)(Y_i - \mu_i)^T$ in the above yields a [sandwich estimate](http://ibg.colorado.edu/cdrom2011/medland/fri2011/HWSE.pdf) $\widehat\Sigma$ of $\Sigma$. The estimate $\widehat\Sigma$ is a consistent estimate of $\Sigma$ even if the working correlation matrices $R_i(\alpha)$ are misspecified. For instance, it is often convenient to use a working independence model where $R_i(\alpha) = I$. Other popular choices include compound symmetry (i.e.,  exchangeable) with $R_{ij} = \alpha$ for any $i \neq j$ or first-order autoregressive with $R_{ij} = \alpha^{|i-j|}$, where $R_{ij}$ denotes the (i,j)th element of R \citep{pan2001akaike}. 
...
As discussed earlier, this difference in interpretation helps explain why GEE coefficient estimates tend to be smaller in magnitude than their GLMM counterparts. This is a major distinction between these two approaches. Feel free to read more about this distinction [here](https://www.jstor.org/stable/25680575?seq=1#metadata_info_tab_contents).
...
To estimate the GEE parameters $(\hat\beta, \hat\alpha, \hat\phi)$, we use an iterative procedure that alternates between updating the regression coefficients $\beta$ and the nuisance parameters $\alpha$ (correlation) and $\phi$ (scale). This general strategy is akin to IRLS in GLMs but extended to account for intra-cluster correlation. More details can be found in \cite{liang1986longitudinal}.

#### Estimating $\beta$

Here, we use a modified IRLS algorithm to update estimation for $\beta$ with $(\hat\alpha,\hat\phi)$ fixed. Given current estimates of the nuisance parameters $\hat\alpha$ and $\hat\phi$ at iteration $k$, we obtain $\hat{\beta}_{k+1}$ as
\begin{equation} \label{eq:betaopt}
  \hat{\beta}_{k+1} = \hat{\beta}_k - \left[\sum_{i=1}^N D_i^T(\hat\beta_k)\widehat{V}_i(\hat\beta_k)D_i(\hat\beta_k)\right]^{-1}\left[\sum_{i=1}^N D_i(\hat\beta_k)\widehat{V}_i(\hat\beta_k)S_i(\hat\beta_k)\right],
\end{equation}
where 
\begin{align*}
...
\end{align*}
This update is equivalent to solving a weighted least squares problem, where the weights reflect both heteroscedasticity and within-cluster correlation.


### Estimating $\alpha$ and $\phi$

Here we use moment based estimation methods to update estimation for $\alpha$ and $\phi$ with $\hat\beta$ fixed. At a given iteration $k$ the correlation parameters $\alpha$ and scale parameter $\phi$ can be estimated from the current Pearson residuals defined by 
$$
  r_{i,t} = \frac{y_{i,t} - \mu_{i,t}(\hat\beta_k)}{\sqrt{ a(\mu_{i,t}(\hat\beta_k))}}
$$
where $i = 1,\ldots,N$ denotes the cluster and $t = 1,\ldots,n_i$ denotes a specific measurement within the cluster.


#### Estimating $\phi$ 

We can estimate the scale parameter as 
$$
  \hat\phi^{-1} = \frac{\sum_{i=1}^N\sum_{t=1}^{n_i} \hat r_{i,t}^2}{K - p}
$$
where $K = \sum n_i$. This is the average squared Pearson residual, scaled by degrees of freedom. This is similar to over-dispersion in GLM. Recall that in a well-specified GLM with correctly modeled variance, the sum of squared Pearson residuals is approximately equal to the degrees of freedom. So if the total is greater than the degrees of freedom, one is  underestimating the variance and can therefore inflate with $\hat\phi$.

#### Estimating $\alpha$

Estimation of the correlation structure relies on empirical correlations of the residuals, computed across individuals. The specific form of the estimator depends on the choice of working correlation matrix $R(\alpha)$. As a general starting point, we compute empirical residual correlations between time points $u$ and $v$,
$$
  \widehat{\mathrm{Corr}}_{u,v} = \frac{1}{N} \sum_{i=1}^N \hat r_{i,u} \hat r_{i,v}.
$$
These are used to construct an estimator $\hat\alpha$ consistent with the assumed structure of $R(\alpha)$. For example, under an AR(1) structure (where $\alpha$ represents the lag-1 autocorrelation), we use
$$
\hat\alpha = \frac{\hat\phi \sum_{i=1}^N \sum_{t=1}^{n_i - 1} \hat r_{i,t} \hat r_{i,t+1}}{K-N}.
$$

This mimics an [empirical autocorrelation](https://stats.stackexchange.com/questions/294407/how-does-the-empirical-correlation-function-differ-from-rs-acf-function), scaled by $\hat\phi$ to account for the variance normalization. This approach works best in settings where all clusters are balanced (i.e., $n_i = n$ for all $i$).

Note that the asymptotic distribution of $\hat\beta$ does not depend on the specific choices of estimators for $\alpha$ and $\phi$ provided that these choices are $\sqrt{N}$ consistent. Remember that estimators $\hat\beta$ and $\widehat V$ will be consistent no matter the choice of $R(\alpha)$. However, choosing $R(\alpha)$ closer to the true correlation gives increased efficiency. 
